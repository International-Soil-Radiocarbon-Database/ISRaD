...
read_YujieHe2016 <- function(Yujie_file = NULL) {
  ...
  Yujie_dataset <- utils::read.csv(Yujie_file,
    na.strings = c("", "NA"),
    ...
    mutate(pro_veg_note = paste(.data$VegTypeCodeStr_Local, .data$VegLocal,
      .data$VegType_Species,
      sep = ";"
    )) %>%
    rename(
      entry_name = .data$pc_dataset_name,
      site_lat = .data$Lat,
      site_long = .data$Lon,
  ...
  ans$profile$pro_land_cover <- stats::setNames(
    land_cover$Controlled,
    land_cover$VegTypeCodeStr_Local
  )[ans$profile$pro_land_cover]


  ## pull in the template
  utils::download.file(
    url = "https://github.com/International-Soil-Radiocarbon-Database/ISRaD/raw/master/inst/extdata/ISRaD_Master_Template.xlsx",
  ...
  template <- lapply(
    list(
      metadata = "metadata", site = "site", profile = "profile",
  ...
  for (currentEntry in as.character(ans$metadata$entry_name)) {
    ...
    openxlsx::write.xlsx(sliceEntry,
      file =
        file.path(
          "~/Dropbox/ISRaD_data/Compilations/Yujie/read_YujiHe2016_out",
