...
# load dataset
Aragones_dataset <- read.csv("/Users/macbook/Desktop/Dropbox Temp/ISRaD/ISCN Collaboration/14C_Dataset_Final_Cristian.csv",
  na.strings = c("", "NA"),
...
Aragones_tidy <- Aragones_dataset %>%
  mutate(
    ...
    Sampling_date = if (length(str_replace_all(Sampling_date, fixed("/"), "")) == 5) {
      mdy(paste("0", str_replace_all(Sampling_date, fixed("/"), "")))
    } else {
...
# read in doi csv
Aragones_doi <- read.csv("/Users/macbook/Desktop/Dropbox Temp/ISRaD/ISCN Collaboration/Estop Aragones DOI List.csv",
  na.strings = c("", "NA"),
  stringsAsFactors = FALSE
)
...
site <- as_tibble(cbind(
  Aragones_gas %>%
    select(
      entry_name = str_replace_all("Study", fixed(" "), "_"),
...
site_note_df <- as_tibble(Aragones_gas) %>%
  mutate(
    Yedoma = as.character(Yedoma),
    Thermokarst = as.character(Thermokarst)
  ) %>%
  mutate(
    Yedoma = replace(Yedoma, Yedoma %in% c("No", "No?"), "Not Yedoma"),
...
site <- site %>%
  group_by(site_name) %>%
  mutate(
    site_latlong = strsplit(site_name[[1]], "  ", fixed = TRUE),
    site_datum = NA,
    site_elevation = NA
  ) %>%
  mutate(
    site_lat = site_latlong[[1]][1],
...
num.aragones.rows <- as_tibble(Aragones_gas) %>%
  ...
  select(
    entry_name, site_name, pro_name_chr,
...
profiles <- as_tibble(cbind(
  sitenames.vector, aragones.rows.vector, entrynames.vector, gral.vector,
...
profiles <- profiles %>%
  ...
  select(
    entry_name, site_name, pro_name_chr, aragones_rows, plot_name,
...
measurements <- measurements %>%
  ...
  mutate(
    measurement_analyte = replace(measurement_analyte, measurement_analyte %in% c("d13C_Atm_CO2", "Fm_Atm_CO2"), "Atm_CO2"),
    ...
    measurement_index = replace(measurement_index, measurement_index %in% c(
      "d13C_Soil", "d13C_Atm_CO2", "d13C_CO2",
      "d13C_CH4", "d13C_DOC", "d13C_POC"
    ), "d13c"),
    measurement_index = replace(measurement_index, measurement_index %in% c(
...
measurements <- measurements %>%
  gather(
    key = "13c or Fm or 2h", value = "value",
    c("flx_2h", "d13c", "Fm")
  ) %>%
  arrange(entry_name, site_name, pro_name) %>%
  filter(!is.na(value)) %>%
...
flux <- flux %>%
  mutate(
    flx_pathway = as.character(measurement_pathway),
    measurement_ecosystem_component = as.character(measurement_ecosystem_component),
    index = x
  ) %>%
  mutate(
    flx_pathway = replace(flx_pathway, measurement_pathway == "Bubbles", "bubble ebullition"),
  ...
  mutate(
    flx_obs_date_y = as.character(flx_obs_date_y),
...
flux <- flux %>%
  ...
  mutate(
    flx_analyte = recode_factor(flx_analyte, Atm_CO2 = "CO2"),
...
layer <- layer %>%
  mutate(
    lyr_name = paste(site_name, pro_name, measurement_depth, sep = "_"),
  ...
  mutate(
    lyr_obs_date_y = as.character(lyr_obs_date_y),
...
layer.red1 <- layer %>%
  ...
  summarize(
    lyr_obs_date_y = lyr_obs_date_y[1],
...
layer.red.final <- bind_rows(layer.red1, layer.soil) %>%
  distinct() %>%
  arrange(entry_name, site_name)
...
interstitial <- interstitial %>%
  ...
  mutate(
    ist_obs_date_y = as.character(ist_obs_date_y),
...
interstitial <- profile[, c(1, 2, 4)] %>%
  inner_join(interstitial) %>%
  distinct()
...
incubation <- incubation %>%
  ...
  mutate(
    ...
    measurement_incubation_soil = replace(
      measurement_incubation_soil, measurement_incubation_soil %in% c("Autotrophic", "Roots"),
      "live roots"
    ),
    measurement_incubation_soil = replace(
      measurement_incubation_soil, measurement_incubation_soil == "Mineral",
      "root-picked soil"
    ),
  ...
  mutate(
    inc_obs_date_y = as.character(inc_obs_date_y),
...
incubation <- layer.red.final[, 1:4] %>%
  inner_join(incubation) %>%
  distinct()
...
layer <- measurements %>%
  ...
  mutate(
    lyr_name = paste(pro_name, measurement_depth, sep = "_"),
    lyr_top = measurement_depth - 5, lyr_bot = measurement_depth + 5,
    lyr_all_org_neg = "yes"
  ) %>%
  select(entry_name,
...
for (i in 1:length(names)) {
  toutput.byentry[[i]] <- list(
    toutput.metadata[[i]], toutput.site[[i]], toutput.profile[[i]], toutput.flux[[i]],
    toutput.layer[[i]], toutput.interstitial[[i]], toutput.fraction[[i]], toutput.incubation[[i]],
    toutput.cvocab[[i]]
  )
}
