...
#' @param template_file Local path to template file
#' @param template__info_file Local path to template info file
#' @import dplyr
#' @importFrom RCurl url.exists
#' @importFrom readxl read_excel excel_sheets
#' @importFrom httr HEAD
#' @export
QAQC_frc <- function(file, writeQCreport = FALSE, outfile_QAQC = "", summaryStats = TRUE,
                 dataReport = FALSE, checkdoi = TRUE, verbose = TRUE, template_file, template_info_file) {
  ...
  template <- ISRaD:::read_template_file(template_file)
  template_info <- ISRaD:::read_template_info_file(template_info_file)
  
  ISRaD:::check_template_info_columns(template, template_info, outfile_QAQC, verbose)
  ...
  mismatch.rows <- anti_join(ISRaD:::lapply_df(data$profile, as.character),
                             ISRaD:::lapply_df(data$site, as.character),
  ...
  if (length(data$flux$entry_name) > 0) {
    ...
    mismatch.rows <- anti_join(ISRaD:::lapply_df(data$flux, as.character),
                               ISRaD:::lapply_df(data$site, as.character),
                               by = c("entry_name", "site_name")
    )
    if (dim(mismatch.rows)[1] > 0) {
  ...
  if (length(data$layer$entry_name) > 0) {
    ...
    mismatch.rows <- anti_join(ISRaD:::lapply_df(data$layer, as.character),
                               ISRaD:::lapply_df(data$profile, as.character),
  ...
  if (length(data$interstitial$entry_name) > 0) {
    ...
    mismatch.rows <- anti_join(ISRaD:::lapply_df(data$interstitial, as.character),
                               ISRaD:::lapply_df(data$profile, as.character),
                               by = c("entry_name", "site_name", "pro_name")
    )
    if (dim(mismatch.rows)[1] > 0) {
  ...
  if (length(data$fraction$entry_name) > 0) {
    ...
    mismatch.rows <- anti_join(ISRaD:::lapply_df(data$fraction, as.character),
                               ISRaD:::lapply_df(data$layer, as.character),
  ...
  if (length(data$incubation$entry_name) > 0) {
    ...
    mismatch.rows <- anti_join(ISRaD:::lapply_df(data$incubation, as.character),
                               ISRaD:::lapply_df(data$layer, as.character),
                               by = c("entry_name", "site_name", "pro_name", "lyr_name")
    )
    if (dim(mismatch.rows)[1] > 0) {
