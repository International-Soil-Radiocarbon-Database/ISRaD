...
#' )
#' }
ISRaD.extra.fill_CN <- function(database, geodata_directory) {
  stopifnot(is_israd_database(database))

  database$layer %>%
    mutate(lyr_fill_c_to_n = ifelse(is.na(lyr_c_to_n), # If yes,
                               ifelse(is.na(lyr_c_inorg), # then check if total C includes inorganic
                                      lyr_c_tot / lyr_n_tot, # if not (is.na(c_inorg) == TRUE), use total C and N to calculate C:N
                                        lyr_c_org / lyr_n_tot), # if inorganic C present (is.na(c_inorg) == FALSE), use organic C only to calculate C:N
                               lyr_c_to_n)) -> database$layer # if c_to_n != NA, then use the existing value and save
