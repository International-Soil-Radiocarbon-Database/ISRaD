...
QAQC <- function(file, writeQCreport=FALSE, outfile_QAQC="", summaryStats=TRUE, dataReport=FALSE, checkdoi=TRUE, verbose=TRUE){
  ...
  if(!grep(".xlsx", file)==1){
    if(verbose) cat("\tWARNING: ", file, " is not the current file type (should have '.xlsx' extension)", file=outfile_QAQC, append = TRUE);error<-error+1
  }
  ...
  if (!all(getSheetNames(file) %in% names(template)) | !all(names(template) %in% getSheetNames(file))){
    if(verbose) cat("\tWARNING:  tabs in data file do not match accepted templates. Please use current template. Visit https://international-soil-radiocarbon-database.github.io/ISRaD/contribute", file=outfile_QAQC, append = TRUE);error<-error+1
  ...
  if(!(all(lapply(data, function(x) x[1,1])=="Entry/Dataset Name") & all(lapply(data, function(x) x[2,1])=="Author_year"))){
    if(verbose) cat("\n\tWARNING:  Description rows in data file not detected. The first two rows of your data file should be the description rows as found in the template file.", file=outfile_QAQC, append = TRUE);error<-error+1
  }
  ...
  for (d in seq_along(dois)){
    if((!(RCurl::url.exists(paste0("https://www.doi.org/", dois[d])) | dois[d] =="israd"))){
      if(verbose) cat("\n\tWARNING: doi not valid", file=outfile_QAQC, append = TRUE);error<-error+1
    }
  }
  }
  } else {
  ...
  for (t in seq_along(names(data))){
    ...
    if (length(notintemplate>0)) {
      if(verbose) cat("\n\tWARNING: column name mismatch template:", notintemplate, file=outfile_QAQC, append = TRUE);error<-error+1
    }
  }
  ...
  for (t in seq_along(names(data))){
    ...
    if (T %in% unlist(missing_values)) {
      if(verbose) cat("\n\tWARNING: missing values where required:", required_colnames[missing_values], "(rows:",which_missing_values+3,")", file=outfile_QAQC, append = TRUE);error<-error+1
    }
  }
  ...
  for (t in seq_along(names(data))){
    ...
    for (c in seq_along(numeric_columns)){
      ...
      if(nonnum) {
        if(verbose) cat("\n\tWARNING non-numeric values in", column, "column", file=outfile_QAQC, append = TRUE); error<-error+1
      } else {
        ...
        if(sum(toobig, na.rm=TRUE)>0) {
          if(verbose) cat("\n\tWARNING values greater than accepted max in", column, "column (rows", which(toobig)+3, ")", file=outfile_QAQC, append = TRUE); error<-error+1
        }

        if(sum(toosmall, na.rm=TRUE)>0) {
          if(verbose) cat("\n\tWARNING values smaller than accepted min in", column, "column (rows", which(toosmall)+3, ")", file=outfile_QAQC, append = TRUE); error<-error+1
        }
  ...
  for (t in 2:length(names(data))){
    ...
    for (c in seq_along(controlled_vocab_columns)){
      ...
      if(F %in% vocab_check){
        if(verbose) cat("\n\tWARNING: unacceptable values detected in the", column, "column:", unique(as.character(data[[tab]][,column][!vocab_check])), file=outfile_QAQC, append = TRUE); error<-error+1
      }
