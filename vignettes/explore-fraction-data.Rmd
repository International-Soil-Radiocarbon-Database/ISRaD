...
### Spread data into multipe columns - Compare fractions from same layer
```{r echo=TRUE, warning = FALSE, message = FALSE}
...
## Make some maps of where the data is located
```{r echo=TRUE, warning = FALSE, message = FALSE}

# Map sites that have fraction data:
map('world')
points(frc_data$site_long, frc_data$site_lat, col=2, pch=19)


#Map sites that have incubation data:
inc.site <- ISRaD_data$incubation %>%
  left_join(ISRaD_data$site) %>%
  filter(is.na(inc_14c) != 1) #filters for data with 14C only
  
map('world')
points(inc.site$site_long, inc.site$site_lat, col=2, pch=19)
